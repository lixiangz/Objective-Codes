...
float helocFinalBalance(const int lineOfCreditMaximum, const float interestRate,
...
float helocFinalBalance(const int lineOfCreditMaximum, const float interestRate,
  ...
  float AmountOwn = initialBalance;
  int count = 0;
  int numMonths = numberOfYears*12;
  while (numMonths > 0){
    
    if (AmountOwn <= 0){
      AmountOwn = AmountOwn - payments[count];
    }
   
    else if (AmountOwn < lineOfCreditMaximum){
      AmountOwn = AmountOwn + ((interestRate/100)/12)*AmountOwn - payments[count];
    }

    if (AmountOwn >= lineOfCreditMaximum){
      AmountOwn = AmountOwn + 0.01*AmountOwn;
    }
...
float totalInterestPaid(const int lineOfCreditMaximum, const float interestRate,
  ...
  while (numMonths > 0){
    if (AmountOwn < 0){
      ...
      interest = interest + ((interestRate/100)/12)*AmountOwn;
      AmountOwn = AmountOwn + ((interestRate/100)/12)*AmountOwn - payments[count];
    }

    if (AmountOwn >= lineOfCreditMaximum){
      interest = interest + 0.01*AmountOwn;
      AmountOwn = AmountOwn + 0.01*AmountOwn;
